      ...
      >
          ...
          <div class="flex items-center justify-between sm:justify-end space-x-3">
            ...
            <div class="flex space-x-2">
              <button
                ...
                class="text-blue-500 hover:text-blue-700 text-xs sm:text-sm px-2 py-1 hover:bg-blue-50 rounded transition-colors"
              >
                Edit
              </button>
              <button
        ...
        <div v-if="editingPostId === post.id" class="mb-3">
          <textarea
            ...
            class="w-full p-3 border border-blue-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm sm:text-base resize-none"
            rows="4"
          ...
          <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 mt-3">
            <button
              ...
              class="w-full sm:w-auto bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 text-sm font-medium transition-colors"
            >
              ✓ Simpan
            </button>
            <button
              @click="cancelEditPost"
              class="w-full sm:w-auto bg-gray-400 text-white px-4 py-2 rounded hover:bg-gray-500 text-sm font-medium transition-colors"
            >
              ✕ Batal
            </button>
          </div>
        </div>
        <p v-else class="mb-3 text-sm sm:text-base leading-relaxed">{{ post.content }}</p>
          ...
          >
              ...
              <div class="flex items-center justify-between sm:justify-end space-x-2">
                ...
                <div class="flex space-x-1">
                  <button
                    ...
                    class="text-blue-500 hover:text-blue-700 text-xs px-1 py-0.5 hover:bg-blue-50 rounded transition-colors"
                  >
                    Edit
                  </button>
                  <button
            ...
            <div v-if="editingReplyId === reply.id" class="mt-2">
              <input
                ...
                type="text"
                class="w-full p-2 border border-blue-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-xs sm:text-sm"
              ...
              <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 mt-2">
                <button
                  ...
                  class="w-full sm:w-auto bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 text-xs font-medium transition-colors"
                >
                  ✓ Simpan
                </button>
                <button
                  @click="cancelEditReply"
                  class="w-full sm:w-auto bg-gray-400 text-white px-3 py-1 rounded hover:bg-gray-500 text-xs font-medium transition-colors"
                >
                  ✕ Batal
                </button>
              </div>
            </div>
            <p v-else class="text-xs sm:text-sm">{{ reply.content }}</p>
          </div>
...
const updatePost = async (postId) => {
  ...
  try {
    await api.put(`/forum/${postId}`, { content: editPostContent.value })
    success.value = 'Post berhasil diperbarui!'
    cancelEditPost()
    fetchPosts()
    clearNotification()
  } catch (err) {
    error.value = err.response?.data?.message || 'Gagal update post'
    clearNotification()
  }
}
...
const updateReply = async (replyId) => {
  ...
  try {
    await api.put(`/forum/${replyId}`, { content: editReplyContent.value })
    success.value = 'Balasan berhasil diperbarui!'
    cancelEditReply()
    fetchPosts()
    clearNotification()
  } catch (err) {
    error.value = err.response?.data?.message || 'Gagal update balasan'
    clearNotification()
  }
}
